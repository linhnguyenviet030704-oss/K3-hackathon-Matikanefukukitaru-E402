...
Đặt `VOYAGE_API_KEY` trong `.env` ở thư mục gốc project. Repository hiện có thể đọc dự phòng `data/.env` nếu root `.env` chưa tồn tại.
...
Chạy lại sau gián đoạn bằng cùng lệnh. Script kiểm tra ID và `_content_hash`, bỏ qua record không đổi, sửa BM25 nếu thiếu và không tạo bản trùng.

ChromaDB nằm ở `data/chroma_db`, collection là `"Skin"`, document embedding dùng `voyage-4` với `input_type="document"`. Vector được lưu bằng HNSW cosine. Giới hạn tốc độ embedding dùng cửa sổ 60 giây với 10.000 TPM, 3 RPM, ngân sách 9.500 token.
...
BM25 dùng SQLite FTS5 tại `data/search_index.sqlite3`, tokenizer `unicode61 remove_diacritics 2`. Rebuild từ ChromaDB:
...
BM25 chỉ dùng tìm kiếm từ khóa, không gọi API:
...
Hybrid lấy BM25 top 10 và vector top 10, chuẩn hóa từng danh sách về `[0, 1]`, rồi tính:
